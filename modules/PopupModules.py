...
from modules.TkModules import make_non_resizable, center_window, Button, Label


def notavideo(invalid_file, valid_file):
    from modules.rootTkSplashModule import root

    longest_invalid_length = max(
        (len(file) for file in invalid_file if len(file) > 50),
        default=0,
    )
    longest_valid_length = max(
        (len(file) for file in valid_file if len(file) > 50),
        default=0,
    )
    ...
    invalid_files_list = emoji.emojize(
        ":cross_mark: ",
    ) + emoji.emojize(
        "\n:cross_mark: ",
    ).join(
    ...
    if len(valid_file) != 0:
        valid_files_list = emoji.emojize(
            ":check_mark_button: ",
        ) + emoji.emojize(
            "\n:check_mark_button: ",
        ).join(
    ...
    errortext = "The following files are not video files:\n\n" f"{invalid_files_list}\n\n" f"{valid_text}"

    display_text_label = Label(
        notavideo,
        text=errortext,
        anchor="center",
        justify="center",
...
def create_popup(root, title, width, height, switch, lift=0):
    ...
    if switch == 1:
        popup.bind(
            "<FocusOut>",
            lambda e: popup.after(200, popup.destroy),
        )
    if lift == 1:
        popup.lift()
